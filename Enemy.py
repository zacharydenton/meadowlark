...
class Enemy(pygame.sprite.Sprite):
	...
	def __init__(self, level, position):
		...
		self.goal					= position
		self.x						= position[0]
		self.y						= position[1] - self.level.enemy_bounds.height
		...
		if Enemy.spritesheet is None:
			Enemy.spritesheet = Spritesheet.Spritesheet(os.path.join('shooter','disasteroids2_master.bmp'))
	...
	def get_image(self,spritesheet,direction):
		return spritesheet.imgat(pygame.rect.Rect(24,291,33,30),-1)
